<?php if ($sp): ?>
	...
	<img src="/img/btn/map.png" alt="Map" class="map-btn" />
	...
	<table class="schedule nav">
		<tr class="header">
			<th colspan="2"><?php
				if ($start === 'next')
				{
					echo '<a class="schedule-btn" data-sp_id="' . $sp['id'] . '" data-when="now" href="' . site_url('pms/service-providers/schedule/' . $sp['id'] . '/now') . '" class="schedule-btn" rel="' . $sp['id'] . '">&lt; This week</a>';
				}
			?></th>
			<th colspan="4">
				<?php echo $title ?>
			...
			<th colspan="2"><?php
				if ($start === 'now')
				{
					echo '<a class="right schedule-btn" data-sp_id="' . $sp['id'] . '" data-when="next" href="' . site_url('pms/service-providers/schedule/' . $sp['id'] . '/next') . '" class="schedule-btn" rel="' . $sp['id'] . '">Next week &gt;</a>';
				}
			?></th>
		</tr>
	</table>
...
<?php else: ?>
	...
	<p>Could not load the given service provider.</p>
