...
class Model_gps extends CI_Model
{
	...
	public function get_gps_ajax()
	{
		$sql = 'SELECT
				...
				AND
					(
						CONVERT(gp_name USING latin1) LIKE "%' . $this->db->escape_like_str($this->input->get('term')) . '%"
						OR
						CONVERT(gp_code USING latin1) LIKE "%' . $this->db->escape_like_str($this->input->get('term')) . '%"
					)
				LIMIT 25';
	...
	public function get_gp($gp_code)
	{
		if($gp_code)
		{
			$sql = 'SELECT
					...
					FROM
						gps
					WHERE
						gp_code = ' . $this->db->escape($gp_code) . '
						AND gp_active = 1';
	...
	public function parse_csv($file_name)
	{
		...
		$handle = fopen($this->config->config['base_dir'] . '/' . $file_name, 'r');
